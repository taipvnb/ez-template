...
using com.ez.engine.assets.loader.resources;
using Sirenix.OdinInspector;
using UnityEngine;
#if USE_ADDRESSABLES
using  com.ez.engine.assets.loader.addressables;
#else
#endif

namespace com.ez.engine.manager.ui
{
	[CreateAssetMenu(fileName = "UISettings", menuName = "EzEngine/View/UI Settings", order = 0)]
	public class UISettings : ScriptableObject
	{
		...
		private static UISettings _default;
		...
		public static UISettings DefaultSettings
		{
			get
			{
				if (_default == false)
				{
					Debug.LogError("UISettings.DefaultSettings is null.");
				}
		...
		public ITransitionAnimation SheetEnterAnimation =>
			_sheetEnterAnimation
				? Instantiate(_sheetEnterAnimation)
				: SimpleTransitionAnimationObject.CreateInstance(beforeAlpha: 0.0f, easeType: EaseType.Linear);

		public ITransitionAnimation SheetExitAnimation =>
			_sheetExitAnimation
